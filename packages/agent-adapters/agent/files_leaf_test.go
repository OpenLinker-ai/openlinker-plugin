...
func TestSharedConfigProductionRoundTripAndRejection(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config", "agent.json")
	...
	config.Provider, config.AgentID = "codex", "12345678-1234-4234-8234-123456789abc"
	config.SessionReuse, config.WebSearch = false, true
	if err := saveConfig(path, config); err != nil {
		t.Fatal(err)
	}
	want, err := json.MarshalIndent(config, "", "  ")
	if err != nil {
		t.Fatal(err)
	}
	raw, err := os.ReadFile(path)
	if err != nil || string(raw) != string(want)+"\n" {
		t.Fatal("persisted config format changed")
	}
	loaded, resolved, err := loadConfig(getenv)
	if err != nil || resolved != path || loaded.SessionReuse || !loaded.WebSearch || loaded.AgentID != config.AgentID {
		t.Fatalf("production config round trip changed: %#v, %v", loaded, err)
	}
	for _, raw := range []string{`{"unknown":true}`, `{"enabled":"false"}`, `{"version":2}`, `{} {}`} {
		if err := os.WriteFile(path, []byte(raw), 0o600); err != nil {
			t.Fatal(err)
		}
		if _, _, err := loadConfig(getenv); err == nil {
			t.Fatalf("invalid persisted config accepted: %s", raw)
		}
		if _, _, err := ConfigureNonSecret(getenv, ConfigureOptions{Provider: "claude"}); err == nil {
			t.Fatal("configuration update overwrote invalid persisted data")
		}
		after, err := os.ReadFile(path)
		if err != nil || string(after) != raw {
			t.Fatal("rejected config update modified persisted bytes")
		}
	}
	// Preserve existing encoding/json behavior; sharing the decoder adds no policy.
	for _, raw := range []string{`null`, `{"enabled":true,"enabled":false}`} {
		if err := os.WriteFile(path, []byte(raw), 0o600); err != nil {
			t.Fatal(err)
		}
		loaded, _, err := loadConfig(getenv)
		if err != nil || loaded.Version != 1 || loaded.Enabled || !loaded.SessionReuse {
			t.Fatalf("legacy decoding/defaults changed: %s, %v", raw, err)
		}
	}
}
